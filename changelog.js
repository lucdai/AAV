        ...
        for (const [category, commits] of Object.entries(entry.categories || {})) {
            if (commits && commits.length > 0) {
                ...
                commits.forEach(commit => {
                    ...
                    const safeUrl = sanitizeUrl(commit.url);
                    html += `
                            ...
                            <div class="flex items-start justify-between gap-2">
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center gap-2 mt-1 text-xs text-slate-500">
                                        <span class="font-mono bg-slate-200 px-2 py-1 rounded">${escapeHtml(commit.sha)}</span>
                                        <span>${escapeHtml(commit.author)}</span>
                                    </div>
                                </div>
                                <a href="${safeUrl}" target="_blank" rel="noopener noreferrer" class="flex-shrink-0 p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all" title="View on GitHub">
...
function addChangelogIcon() {
    ...
    if (githubLink && !document.getElementById('changelogIcon')) {
        ...
        changelogIcon.href = '#';
        changelogIcon.className = 'inverse-system-icon-btn w-10 h-10 rounded-full flex items-center justify-center transition-all cursor-pointer';
